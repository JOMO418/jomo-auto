...
export function AddSingleModal({ onClose, onCreated }: {
  onClose: () => void;
  ...
  // DB-driven vehicle IDs (replaces static VEHICLE_GROUPS)
  ...
  const [form, setForm] = useState({
    ...
    condition: 'Ex Japan', origin: 'Japan', category: '',
  ...
  async function handleSave() {
    ...
    try {
      ...
      const payload = {
        ...
        images: allImages,
        // Send vehicle UUIDs — API will resolve to strings automatically
      ...
      onCreated({
        id: data.id, slug: '', category_id: '', specs: {},
        ...
        flash_deal: payload.flash_deal, images: allImages,
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/75 backdrop-blur-sm p-0 sm:p-3">
      <div className="bg-[#111] border border-white/[0.08] w-full sm:max-w-4xl rounded-t-3xl sm:rounded-2xl flex flex-col max-h-[97dvh] sm:max-h-[96vh] shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="flex items-center justify-between px-5 pt-4 pb-3 flex-shrink-0">
          <div>
            <h2 className="text-white font-bold text-[15px] leading-none">Add Product</h2>
            <p className="text-zinc-500 text-[11px] mt-0.5">{form.name || 'New product'}</p>
          </div>
            ...
            <>
              ...
              <button type="button" onClick={handleSave} disabled={saving} className="flex-1 py-3 rounded-xl bg-[#E8002D] hover:bg-[#c5001f] disabled:opacity-60 text-white text-sm font-bold transition-colors flex items-center justify-center gap-2">
                {saving
                  ...
                  : <><CheckCircle2 className="h-4 w-4" />Save Product</>
                }
              </button>
            </>
          )}
